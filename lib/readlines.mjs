...
function _concat(buffOne, buffTwo) {
    if (!buffOne && !buffTwo) {
        throw new Error('when concatenating two buffers, at least one buffer but exist');
    }
...
export function* readlines(
    fd,
    filesize,
    { bufferSize = 64 * 1024, position = 0, maxLineLength = Infinity } = {}
) {
    if (maxLineLength <= 0) {
    ...
    while (position < filesize) {
        ...
        while (curpos < bytesRead) {
            ...
            // yield the buffer
            const wantedLength = yield buffer;
 ...
 * @return {Object} The generator object
 */
export function* fromFile(filename, { bufferSize, maxLineLength } = {}) {
    const fd = fs.openSync(filename, 'r');
    const fileSize = fs.statSync(filename).size;

    for (let line of readlines(fd, fileSize, { bufferSize, maxLineLength })) {
        yield line.toString();
    }
